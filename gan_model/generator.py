...
def generatorModel(sizeSeedGenerator, sizeImage, addCon2dLayers=False, coefFilters=32):
    minimalSize = int(round(sizeImage[0] / 8))
    numberLayers, sizeReduce = findMaxDivisorBy2(sizeImage[0], minimalSize)

    model = Sequential()
    model.add(Dense(sizeReduce * sizeReduce * coefFilters * (numberLayers), activation=LeakyReLU(alpha=0.2),
                    input_shape=(sizeSeedGenerator,)))
    model.add(BatchNormalization())
    model.add(Reshape((sizeReduce, sizeReduce, coefFilters * (numberLayers))))
